...
Current application version: `1.0.15`
...
- `BOOTSTRAP_BRANCH`: clone from a different Git branch
...
- Deployment setup keeps common work in `setup.sh` and platform-dependent work in `ol8/`, `ol9/`, `ubuntu/`, and `macos/` setup modules.
- Firewall setup is best-effort. On Oracle Linux, the platform module uses simple firewalld commands: enable/start firewalld, read the active zone or default zone, add the selected listener port permanently, reload, list services/ports, and verify the listener with `ss`. It does not restart DBus or run fallback firewall frameworks. On Ubuntu OCI images without `ufw`, setup falls back to `iptables` and persists the rule to `/etc/iptables/rules.v4` when that path is available.
- On Ubuntu, the platform module handles AppArmor for the embedded app-local MySQL Server when `/etc/apparmor.d/usr.sbin.mysqld` exists by adding local allowances for generated `etc/my.cnf`, `.embedded/mysql-server/`, and `.data/`, then reloading the profile. If that MySQL profile is absent, setup logs the skip and continues.
