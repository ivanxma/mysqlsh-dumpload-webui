...
- Firewall setup is best-effort. On Oracle Linux, `setup.sh` uses bounded `firewall-cmd` calls and prints a manual follow-up if firewalld does not respond, so OCI init does not remain stuck in the installing state. On Ubuntu OCI images without `ufw`, setup falls back to `iptables` and persists the rule to `/etc/iptables/rules.v4` when that path is available.
