...
- Firewall setup is best-effort. On Oracle Linux, `setup.sh` uses bounded `firewall-cmd` calls and prints a manual follow-up if firewalld does not respond, so OCI init does not remain stuck in the installing state.
