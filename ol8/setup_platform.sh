...
platform_resolve_firewalld_zone() {
  ...
  echo "Unable to read OL8 active firewalld zone; restarting firewalld and checking DBus." >&2
  run_as_root systemctl restart firewalld || true
  run_as_root systemctl is-active --quiet dbus || {
    echo "OL8 DBus is not active; restarting dbus before retrying firewalld." >&2
  ...
  if [[ -n "$zone" ]]; then
    echo "Using OL8 default firewalld zone after active-zone lookup was unavailable: $zone" >&2
    printf '%s' "$zone"
    return 0
  fi
